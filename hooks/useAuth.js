...
export function useLogin() {
  ...
  return useMutation({
    mutationKey: ["login"],
    mutationFn: (data) =>
      apiClient.invokeFunction({ method: "auth_login", data }),
    onSuccess: async (data) => {
      ...
      const userData =
        responseData?.user_data || responseData?.userData || responseData?.user;
      ...
      try {
        if (
          responseData?.role?.name !== "plan_fakt_admins" &&
          branches.length > 0
        ) {
        ...
        if (responseData?.role?.name === "employees") {
          if (permissions?.data?.data?.role_permissions) {
            appStore.setNewPermission(
              permissions?.data?.data?.role_permissions
            );
          } else {
...
export function useRegister() {
  ...
  return useMutation({
    ...
    onError: (error) => {
      ...
      const errorMessage =
        typeof apiError === "string" && apiError.toLowerCase().includes("exist")
