      ...
      <tr className={`border-b border-gray-100 transition-colors duration-200 hover:bg-[#f0f4f8] ${isTotalRow ? 'font-semibold' : ''}`}>
        ...
        >
          ...
          >
            ...
            <span className={isTotalRow ? 'font-semibold' : ''}>{row?.name}</span>
          </div>
        </td>
        {columns.map(column => (
          <td
            key={column.key}
            className={`px-2 py-1.5 text-xs text-slate-900 border-b border-gray-200 text-right font-semibold whitespace-nowrap min-w-[120px] ${isActiveOrPassive && 'bg-primary! text-white!'}`}
          >
...
export default observer(function BalanceTestPage() {
  ...
  const toggleRow = (uniquePath) => {
    setExpandedRows(prev => {
      ...
      if (next.has(uniquePath)) { next.delete(uniquePath) } else { next.add(uniquePath) }
      return next
    })
  }
