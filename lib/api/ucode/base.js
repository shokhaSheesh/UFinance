...
class UcodeAPIClient {
  ...
  async refreshAccessToken() {
    ...
    this._refreshPromise = (async () => {
      try {
        const refreshToken = typeof window !== 'undefined' && authStore.refreshToken
  ...
  async getSinglePageBody(id) {
    try {
      const url = `${this.baseURL}/v2/object-slim/get-list/wlcm_links?project-id=${this.projectId}&data=${encodeURIComponent(JSON.stringify({ guid: id, }))}`
  ...
  async invokeFunction({ method, data = {}, authToken: providedToken = null, headers: additionalHeaders = {}, type = "" }) {
    ...
    const authToken = (method === 'auth_register_legal_entity' || method === 'auth_login') ? '' : this.getAuthToken(providedToken)
  ...
  async defaultUcodeFunction({ urlMethod = '', urlParams = '', data = {}, id }) {
