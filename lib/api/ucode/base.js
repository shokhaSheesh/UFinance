...
class UcodeAPIClient {
  constructor() {
    ...
    this.invokeFunctionGetWayEndpoint = '/v2/invoke_function/planfact-function-getawey/v1'
  ...
  async defaultUcodeFunction({ urlMethod = '', urlParams = '', data = {}, id }) {
    ...
    const url = `${this.baseURL}/v2${urlParams}` + (urlMethod === 'GET' ? `&data=${encodeURIComponent(JSON.stringify({ branch_id: authStore.branch_id, plan_fakt_admins_id: authStore?.userData?.guid, }))}` : '')
