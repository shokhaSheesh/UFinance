...
class UcodeAPIClient {
  constructor() {
    ...
    this.invokeFunctionEndpoint =
      process.env.NEXT_PUBLIC_INVOKE_FUNCTION_ENDPOINT;
    this.invokeFunctionGetWayEndpoint =
      process.env.NEXT_PUBLIC_INVOKE_FUNCTION_GETAWAY_ENDPOINT;
    this.projectId = "3ed54a59-5eda-4cfe-b4ae-8a201c1ea4ed";
    this.environmentId = process.env.NEXT_PUBLIC_ENVIRONMENT_ID;
  ...
  async refreshAccessToken() {
    ...
    this._refreshPromise = (async () => {
      try {
        const refreshToken =
          typeof window !== "undefined" && authStore.refreshToken;
        ...
        authStore.setToken("authToken", newAccessToken);
        if (typeof window !== "undefined") {
          localStorage.setItem("authToken", newAccessToken);
        }

        if (newRefreshToken) {
          authStore.setToken("refreshToken", newAccessToken);
  ...
  buildInvokeFunctionBody(method, data = {}) {
    ...
    // Явно переданный branch_id важнее сохранённого: webview переклички
    // работает без логина и присылает филиал в теле запроса
    if (authStore?.branch_id && !requestData.branch_id) {
  ...
  async getSinglePageBody(id) {
    try {
      const url = `${
        this.baseURL
      }/v2/object-slim/get-list/wlcm_links?project-id=${
        this.projectId
      }&data=${encodeURIComponent(JSON.stringify({ guid: id }))}`;
      ...
      if ((data?.data?.error || data?.error) && !response.ok) {
        throw { error: "" };
      }

      const result = data?.data?.data?.response?.[0];

      return {
        amount: result?.amount / 100,
        guid: result?.guid,
        qrcode: result?.qrcode,
        salesId: result?.sales_transactions_id,
      };
    } catch (error) {
      console.log("getSinglePageBody", error);
    }
  }
  ...
  async handleResponse(response, method) {
    ...
    if (
      ...
      responseData.status === "INVALID_ARGUMENT" ||
    ...
    if (!response.ok) {
      // console.log('response not ok base.js', responseData)
  ...
  async invokeFunction({
    ...
    const authToken =
      method === "auth_register_legal_entity" || method === "auth_login"
        ? ""
        : this.getAuthToken(providedToken);
    ...
    const baseUrl =
      ...
      `${this.baseURL}${
        type === "role"
          ? this.invokeFunctionGetWayEndpoint
          : this.invokeFunctionEndpoint
      }?project-id=3ed54a59-5eda-4cfe-b4ae-8a201c1ea4ed`;
    const url = baseUrl;

    const headers = this.buildHeaders(authToken, additionalHeaders);

    // Для auth методов добавляем environment-id в заголовки.
    // Без токена шлюз тоже пропускает запрос только с environment-id, иначе
    // отвечает 400 BAD_REQUEST — это случай webview переклички в телеграме,
    // где логина и localStorage с токеном нет
    if (isAuthMethod || !authToken) {
    ...
    try {
      ...
      // Без токена обновлять нечего (webview): отдаём ответ как есть
      if (response.status === 401 && !isAuthMethod && authToken) {
  ...
  async defaultUcodeFunction({
    ...
    let url = `${this.baseURL}/v2${urlParams}`;
    if (urlMethod === "GET") {
      const parsedUrl = new URL(url);
      const searchParams = parsedUrl.searchParams;
      let existingData = {};
      const existingDataParam = searchParams.get("data");
      if (existingDataParam) {
        try {
          existingData = JSON.parse(decodeURIComponent(existingDataParam));
        } catch (e) {
          existingData = {};
        }
      }
      ...
      const mergedData = {
        ...
        plan_fakt_admins_id: authStore?.userData?.guid,
      };

      if (!authStore.userData?.can_view_all_branches) {
        mergedData.branch_id = authStore.userData?.branch_id;
      }
      if (authStore?.branch_id) {
        mergedData.branch_id = authStore.branch_id;
      }

      searchParams.set("data", JSON.stringify(mergedData));
    ...
    try {
      ...
      if (response.status === 500) {
        const result = await response.json();
        if (typeof window !== "undefined" && result?.status?.startsWith("UN")) {
          authStore.logout();
...
export function createCRUDMethods(resourceName) {
  ...
  return {
    ...
    update: async (data) => {
      if (!data.guid)
        throw new Error(`${capitalizedName} GUID is required for update`);
    ...
    delete: async (guids) => {
      ...
      if (guidArray.length === 0)
        throw new Error(`At least one ${capitalizedName} GUID is required`);
 ...
 */
export async function handleUcodeResponse(
  response,
  endpointName = "U-code API"
) {
...
export async function makeUcodeGetRequest({
  ...
  try {
    ...
    console.error(
      "U-code POST API network error (table structure):",
      networkError
    );
    // Re-throw as a structured error that will be caught by API route
    throw {
