 ...
 */
export const aiChatSendMessage = ({ content, files = [], branchId, chatId } = {}) =>
  aiChatRequest("ai_chat_send_message", {
    content,
    // вложения уходят отдельным полем, а не ссылками внутри текста
    files,
...
/**
 * Удалить чат целиком (вместе с перепиской).
 */
export const aiChatDelete = ({ chatId, branchId } = {}) =>
  aiChatRequest("ai_chat_delete", {
    branch_id: branchId || authStore?.branch_id,
    chat_id: chatId,
  });
