 ...
 * ============================================
 * Конверт: { data: { auth, method, object_data } }, `branch_id` — внутри
 * object_data. `app_id` в теле не передаётся: проект задаётся query-параметром
 * `project-id` эндпоинта.
 * Эндпоинт — invoke_function (functionsV2).
 * Описание методов: BUDGETS_API.md
 */

// project-id в query эндпоинта (см. lib/config/api.js)
 ...
 * Без `date` сумма пишется на весь период бюджета — режим «Итого»,
 * доступный при `plan_total_active` в общих настройках.
 */
export const createBudgetPlan = ({ budgets_id, id, date, amount }) =>
  budgetsRequest('create_budget_plan', {
    budgets_id,
    id,
    ...(date ? { date } : {}),
    amount,
