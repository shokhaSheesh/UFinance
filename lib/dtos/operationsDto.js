...
const operationsDto = (operations, type) => {
	...
	return operations
		...
		.map(item => ({
			...
			projects_id: item?.projects_id || null,
			projectName: item?.projects_name || item?.project_name || item?.projects_id_data?.nazvanie || '',
			...
			// Номер платёжного поручения — печатается под статьёй, когда включена
			// настройка show_payment_order_number. Бэк отдаёт его под разными
			// именами (и пока не отдаёт вовсе), поэтому принимаем несколько вариантов.
			paymentOrderNumber:
				item?.nomer_platezhnogo_porucheniya ??
				item?.payment_order_number ??
				item?.nomer_platezhki ??
				item?.nomer_pp ??
				item?.nomer_dokumenta ??
				null,
			debit: item?.debit,
			kredit: item?.kredit,
			...
			purchase_transaction_name: item?.purchase_transaction_name,
			purchase_transaction_name_2: item?.purchase_transaction_name_2,
			purchase_transactions_id: item?.purchase_transactions_id,
			purchase_transactions_id_2: item?.purchase_transactions_id_2,
			currency:
				item?.currenies_kod || item?.currenies_symbol || item?.currenies_id_data?.nazvanie || '',
			currencySymbol: item?.currenies_kod || item?.currenies_symbol || '',
			...
			operationType: item.tip?.[0] === 'Поступление' ? 'income' : item.tip?.[0] === 'Выплата' ? 'payment' : item.tip?.[0] === 'Перемещение' ? 'transfer' : item.tip?.[0] === 'Отгрузка' ? 'shipment' : item.tip?.[0] === 'Поставка' ? 'supply' : 'accrual',
			...
			operationParts: item?.operationParts?.map(child => ({
				...
				projects_id: child?.projects_id || null,
				projectName: child?.projects_name || child?.project_name || child?.projects_id_data?.nazvanie || '',
				...
				percent: calculatePercent(item?.summa, child?.summa),
