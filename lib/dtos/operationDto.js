...
const operationDto = (operation) => {
  ...
  return {
    ...
    chartOfAccounts:
      operation.chart_of_accounts_name || operation.chart_of_accounts_id_data?.nazvanie || '-',
    chartOfAccounts2:
      operation.chart_of_accounts_name_2 || operation.chart_of_accounts_id_2_data?.nazvanie || '-',
    ...
    currency:
      operation?.currenies_symbol || operation?.currenies_kod || operation?.currenies_id_data?.nazvanie || '',
    currencySymbol: operation?.currenies_symbol || operation?.currenies_kod || '',
    ...
    operationType: operation.tip?.[0] === 'Поступление' ? 'income' : operation.tip?.[0] === 'Выплата' ? 'payment' : operation.tip?.[0] === 'Перемещение' ? 'transfer' : operation.tip?.[0] === 'Отгрузка' ? 'shipment' : operation.tip?.[0] === 'Поставка' ? 'supply' : 'accrual',
    to_currenies_kod: operation?.to_currenies_symbol || operation?.to_currenies_kod,
