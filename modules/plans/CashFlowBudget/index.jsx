...
const CashFlowBudget = () => {
  const t = useTranslations('Plans.cashFlowBudget')
  ...
  // Filter and sort data using useMemo instead of useEffect
  ...
  return (
    <div className="flex flex-col h-full bg-white ">
      ...
      <div className="flex items-center justify-between px-6 py-4 ">
        ...
        <div className="flex items-center gap-3">
          <SelectMyAccounts
            ...
            placeholder={tc('placeholders.selectAccount')}
            multi={false}
      ...
      {/* Table Header - Using divs instead of table */}
      <div className='px-4'>
        <div className="flex  items-center bg-neutral-100 border-b border-neutral-50 text-xs font-medium text-neutral-500 sticky top-0 z-10">
          <div
      ...
      {/* Table Body - Using divs instead of table */}
        ...
        {filteredData.length === 0 ? (
          ...
          <div className="flex flex-col">
            {filteredData.map((item, index) => (
              <div
                key={item.id}
                className={`flex items-center text-sm border-b border-gray-100 hover:bg-blue-50 transition-colors cursor-pointer ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'
                  }`}
              >
                <div className="px-3 py-2 w-[300px] line-clamp-1 font-medium text-slate-900">
                  {item.name}
                </div>
                <div className="px-3 py-2 w-[80px] text-start text-gray-600 font-medium">
                  {item.currency}
                </div>
                <div className="px-3 py-2 flex-1 min-w-[140px] text-gray-600">
                  {item.legalEntity || '—'}
                </div>
                <div className="px-3 py-2 flex-1 min-w-[140px] text-gray-600">
                  {item.project || '—'}
                </div>
                <div className="px-3 py-2 w-[140px] text-gray-600 text-xs">
                  {item.period}
                </div>
                <div className="px-3 py-2 w-[120px] text-gray-600 text-xs">
                  {item.modifiedDate}
                </div>
                <div className="px-3 py-2 flex-1 min-w-[160px] text-gray-600 text-xs">
                  {item.modifiedBy}
                </div>
      ...
      <div className="flex items-center justify-between px-6 py-3 border-t border-gray-200 bg-gray-50 text-xs text-gray-600">
        <span>
          {t('footer.total', { count: filteredData.length })}
        </span>
