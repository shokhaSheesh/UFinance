...
export function useProductServiceData(t, tc) {
  ...
  const { data: productServices, isLoading } = useUcodeRequestQuery({
    ...
    querySetting: { select: data => data?.data }
  })
  ...
  const productServicesList = useMemo(() => {
    ...
    rawList.forEach(item => {
      if (!groupsMap.has(item.groupId)) {
        groupsMap.set(item.groupId, { isGroup: true, guid: item.groupId, name: item.groupName, items: [] })
      }
  ...
  const totalItemsCount = useMemo(() => productServices?.total, [productServices])
