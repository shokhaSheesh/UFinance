...
const ProductServiceRow = ({
  ...
  <tr className="hover:bg-neutral-50 border-b border-gray-200 text-sm">
    ...
    <td className="p-3 text-end text-neutral-700">{item?.price ? `${formatNumber(item.price)} ${item?.currency || ''}` : '—'}</td>
    <td className="p-3 text-center text-neutral-500">{item?.unit}</td>
    <td className="p-3 text-center text-neutral-500">{item?.vat}</td>
    <td className="p-3 text-end text-neutral-700">{item?.priceWithVat ? `${formatNumber(item.priceWithVat)} ${item?.currency || ''}` : '—'}</td>
          ...
          <DropdownMenuContent className="w-40 p-2" align="end">
            {permissions.edit && (
              <DropdownMenuItem asChild>
                <button className={cn("w-full flex items-center cursor-pointer text-sm gap-2 pb-2 justify-start outline-none")} onClick={() => onEdit(item)}>
                  <Pencil size={16} />
                  <span>{tc('edit')}</span>
                </button>
              </DropdownMenuItem>
            ...
            {permissions.add && (
              <DropdownMenuItem asChild>
                <button className={cn("w-full flex items-center cursor-pointer text-sm gap-2 pb-2 justify-start outline-none")} onClick={() => onCopy(item)}>
            ...
            {permissions.delete && (
              <DropdownMenuItem asChild>
                <button className={cn("w-full flex items-center text-red-500 cursor-pointer text-sm gap-2 justify-start outline-none")} onClick={() => onDelete(item)}>
