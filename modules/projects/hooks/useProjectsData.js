...
export function useProjectGroups() {
  ...
  const groups = (query.data?.data || []).map((g) => ({ value: g.guid, label: g.name }))
  return { ...query, groups }
}
...
export function useProjectOptions() {
  ...
  const options = (query.data?.data || []).map((p) => ({ value: p.guid, label: p.name }))
...
export function useProject(guid) {
  ...
  return { ...query, project: query.data?.data ? normalizeProject(query.data.data) : null }
}
...
export function useUpdateProject() {
  ...
  return useMutation({
    ...
    onSuccess: (_d, vars) => {
      invalidate()
      if (vars?.guid) queryClient.invalidateQueries({ queryKey: ['get_project', vars.guid] })
    },
    onError: (e) => showErrorNotification(e?.message),
  })
}
...
export function useCreateProjectGroup() {
  ...
  return useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['list_project_groups'] }),
    onError: (e) => showErrorNotification(e?.message),
  })
}
