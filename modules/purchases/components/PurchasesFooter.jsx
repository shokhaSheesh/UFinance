...
export default function PurchasesFooter({ t, summary, totalProfit, isFilterOpen }) {
  return (
    <footer
      className={cn(
    ...
    >
      <span className="flex items-center gap-1.5">
        <span className="text-[11px] text-gray-500 font-medium">
          {t('footer.dealsCount', { count: summary?.total || summary?.count || 0 })}
        </span>
        <span className="text-xs font-semibold text-slate-900">
          {formatAmount(summary?.total_deal_amount || summary?.total_deals_sum || 0)}
        </span>
